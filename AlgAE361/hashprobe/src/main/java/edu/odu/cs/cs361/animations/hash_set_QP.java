...
//
//      Hash table implementation of set (Quadratic Probing)
//
...
//!public:
  hash_set_QP () { table = new ArrayList<HashEntry>(); theSize=(0); //!  hash_set (): table(hSize), theSize(0) 
  ...
  boolean insert (T element)//!  bool insert (const T& element)
  {
	     ...
	     && count < hSize)//!               && count < hSize)
      {
	    ...
	    ++count;
	    arec.var("count", count);//!
	    h.set((h0.get() + /*f(count)*/ count*count) % hSize);//!	    h = (h0 + /*f(count)*/ count*count) % hSize;
